...
from utils_svces.functions import (
    n_restarts,
    noise_magnitute,
    blockPrint,
    comp_logical_tensors,
)
...
def get_norm(norm_name):
    return norms_dict_torch.get(norm_name, (lambda x: x)(float_(norm_name)))
...
def get_loss(
    ...
    if kwargs_["d_config"].dataset.lower() == "celeba":
        classifier_loss = -F.binary_cross_entropy_with_logits(
            out_, targets_, reduction=reduction
        )
    elif (
        kwargs_["RATIO_config"].activate or kwargs_["RATIO_config"].frank_wolfe.activate
    ):
        # ConfDiff loss
        # -
        ...
        constraint = (
            kwargs_["inverse_config"].inverse_mode_threshold_probs
            - torch.softmax(out_, dim=1).gather(1, targets_.reshape(-1, 1)).squeeze()
        ).squeeze()
        ...
        if kwargs_["inverse_config"].type == "not_dynamic_penalty":
            ...
            if kwargs_["inverse_config"].norm == "l2":
                ...
                minimization_norm = reduce_tensor(
                    -dist_penalty
                    * (x_0 - x_new).reshape(x_0.shape[0], -1).norm(p=2, dim=1) ** 2,
                    reduction=reduction,
                )
            ...
            elif kwargs_["inverse_config"].norm in [
                "LPIPS",
                "FeatureDist",
                "MS_SSIM_L1",
            ]:
                minimization_norm = reduce_tensor(
                    -dist_penalty * loss_fn_LPIPS(x_0, x_new).squeeze(),
                    reduction=reduction,
                )
            else:
                try:
            ...
            if (
                c is not None
                and (c + 1) % kwargs_["inverse_config"].check_steps == 0
                and not no_update
            ):
                improved_constraint = (
                    constraint.detach()
                    <= kwargs_["inverse_config"].constr_improvement_rate
            ...
            if mask is None:
                ...
                penalty = reduce_tensor(
                    -kwargs_["inverse_config"].penalty_multiplier
                    * (torch.nn.functional.relu(constraint)),
                    reduction=reduction,
                )  # - 1000 * cos_loss, reduction=reduction)
            else:
            ...
            classifier_loss = penalty + (
                0 if first_sample else minimization_norm
            )  # + penalties_box_1 + penalties_box_0
            ...
            print(
                "penalty multi",
                kwargs_["inverse_config"].penalty_multiplier.shape,
                kwargs_["inverse_config"].penalty_multiplier,
            )
            ...
            print(
                "probs check penalty mutli",
                kwargs_["inverse_config"].penalty_multiplier,
            )
            print(
                "probs check lagrangian multi",
                kwargs_["inverse_config"].lagrange_multiplier,
            )
            print(
                "prob origin",
            ...
            if kwargs_["inverse_config"].norm == "l2":
                minimization_norm = reduce_tensor(
                    -dist_penalty
                    * (x_0 - x_new).reshape((x_0 - x_new).shape[0], -1).norm(p=2, dim=1)
                    ** 2,
                    reduction=reduction,
                )  # (diff.reshape(diff.shape[0], -1)**2).sum(dim=1)
            elif kwargs_["inverse_config"].norm == "l1":
                ...
                minimization_norm = reduce_tensor(
                    -dist_penalty * loss_fn_LPIPS.forward(x_0, x_new).squeeze(),
                    reduction=reduction,
                )
            else:
            ...
            # print('minimization norm', minimization_norm)
            if first_sample:
                classifier_loss = reduce_tensor(-penalty, reduction=reduction)
                ...
                if c is not None and not no_update:
                    new_lagrage_mult = torch.autograd.grad(
                        penalty.sum(), constraint, retain_graph=True
                    )[0]
                    kwargs_["inverse_config"].lagrange_multiplier.mul_(
                        kwargs_["inverse_config"].ema_weight
                    ).add_(
                        new_lagrage_mult, alpha=1 - kwargs_["inverse_config"].ema_weight
                    ).clamp_(
                        1e-6, 1e6
                    )

                penalty_final = penalty_func(
                    constraint,
                    kwargs_["inverse_config"].penalty_multiplier,
                    kwargs_["inverse_config"].lagrange_multiplier,
                )
                ...
                classifier_loss = (
                    reduce_tensor(-penalty_final, reduction=reduction)
                    + minimization_norm
                )
...
def anneal_Langevin_dynamics(
    ...
    **kwargs,
    ...
    def batch_view(tensor):
        return tensor.reshape(batch_size, *[1] * (x_mod_.ndim - 1))
    ...
    least_norm_value = torch.full(
        (batch_size, 1), 1e10, device="cpu", dtype=torch.float
    )
    ##least_cos_value = torch.full((batch_size, 1), 1e10, device='cpu', dtype=torch.float)

    max_confidence_value = torch.full(
        (batch_size, 1), 0, device="cpu", dtype=torch.float
    )
    max_confidence_value_not_reached_threshold = torch.full(
        (batch_size, 1), 0, device="cpu", dtype=torch.float
    )

    max_confidence_iterate = torch.full(
        (batch_size, 1), 0, device="cpu", dtype=torch.float
    )
    max_confidence_iterate_not_reached_threshold = torch.full(
        (batch_size, 1), 0, device="cpu", dtype=torch.float
    )
    ...
    if type(kwargs["inverse_config"].inverse_mode_threshold_probs) != torch.Tensor:
        inverse_threshold = torch.cuda.FloatTensor(
            [kwargs["inverse_config"].inverse_mode_threshold_probs], device=device
        )
    else:
    ...
    if kwargs["d_config"].dataset.lower() == "oct":
        ...
        pass
    target_classes = (
        kwargs["target_classes"].to(device)
    ...
    if True:  # kwargs['norm'] in ['LPIPS']:
        ...
        loss_fn_LPIPS = lpips.LPIPS(
            net="vgg"
        )  # if kwargs['regularizer'] or kwargs['inverse_config'].activate else 'alex')
        ...
        if kwargs["FeatureDistmodel"] is not None:
            ...
            def loss_fn_FeatureDist(x, y):
                return torch.norm((x - y).reshape(batch_size, -1), p=2, dim=1)
        ##loss_fn_MS_SSIM_L1_ = MS_SSIM_L1_LOSS()
        def loss_fn_MS_SSIM_L1(x, y):
            return torch.norm((x - y).reshape(batch_size, -1), p=2, dim=1).sum(1)  ##loss_fn_MS_SSIM_L1_(x, y).reshape(batch_size, -1).sum(1)
        ...
        def dist_LPIPS(x, y, mean=True):
            return torch.norm((x - y).reshape(batch_size, -1), p=2, dim=1).sum(1)  # lambda x, y, mean=True: loss_fn_LPIPS.forward(x, y).mean(0) if mean else loss_fn_LPIPS.forward(x,
        #  y).cpu()
        if kwargs["FeatureDistmodel"] is not None:
            def FeatureDist_fun(x, y, mean=True):
                return loss_fn_FeatureDist.forward(x, y).mean(0) if mean else loss_fn_FeatureDist.forward(x, y).cpu()
        else:
            def FeatureDist_fun(x, y, mean=True):
                return loss_fn_FeatureDist(x, y).mean(0) if mean else loss_fn_FeatureDist(x, y).cpu()

        def dist_MS_SSIM_L1(x, y, mean=True):
            return loss_fn_MS_SSIM_L1(x, y).mean(0) if mean else loss_fn_MS_SSIM_L1(x, y).cpu()

    # if kwargs['norm'] in ['LPIPS'] or p == 'LPIPS'
    def _view_norm(x, y, p=get_norm(kwargs["norm"]), mean=True):
        return torch.sqrt(dist_LPIPS(x, y, mean)) if p == "LPIPS" else torch.sqrt(FeatureDist_fun(x, y, mean)) if p == "FeatureDist" else dist_MS_SSIM_L1(x, y, mean) if p == "MS_SSIM_L1" else torch.norm((x - y).reshape(batch_size, -1), p=p, dim=1)

    def norms(x_new, x_start):
        return _view_norm(x_new, x_start, 0).unsqueeze(1), _view_norm(x_new, x_start, 1).unsqueeze(1), _view_norm(x_new, x_start, 2).unsqueeze(1), _view_norm(x_new, x_start, float("inf")).unsqueeze(1), _view_norm(x_new, x_start, 1).unsqueeze(1), _view_norm(x_new, x_start, 2).unsqueeze(1), _view_norm(x_new, x_start, float("inf")).unsqueeze(1)  # ,
    ...
    kwargs["inverse_config"].penalty_multiplier = torch.full(
        (batch_size,),
        kwargs["inverse_config"].penalty_multiplier_init,
        device=device,
        dtype=torch.float,
    )
    ...
    with torch.enable_grad():
        ...
        target_one_hot = F.one_hot(
            kwargs["target_classes"], len(kwargs["d_config"].class_labels)
        )
        # second probable class
        out_0_grad = (
            out_0.where(
                ~target_one_hot.bool(), torch.tensor(-np.float("inf")).to(device)
            )
            .max(dim=1)
            .values
        )
        ...
        kwargs["inverse_config"].out_0_grad = x_mod_.grad.data.detach().reshape(
            batch_size, -1
        )
        kwargs["inverse_config"].out_0_grad /= (
            kwargs["inverse_config"].out_0_grad.norm(p=2, dim=1).reshape(batch_size, 1)
        )
        # x_mod_.grad.data.zero_()
        print(
            "out_0_grad",
            len(kwargs["inverse_config"].out_0_grad),
            kwargs["inverse_config"].out_0_grad,
        )
        ##kwargs['inverse_config'].out_temp_grad = x_mod_.grad.data

    classifier_loss_0 = get_loss(
        kwargs_=kwargs,
        out_=out_0,
        targets_=target_classes,
        reduction="none",
        x_0=x_0,
        x_new=x_mod_,
        first_sample=True,
    ...
    objective_vals.append(torch.unsqueeze(classifier_loss_0.detach(), 1).cpu())
    print("start loss 1", objective_vals)
    logits.append(out_0.gather(1, target_classes.reshape(-1, 1)).cpu())
    probs.append(
        torch.softmax(out_0, dim=1).gather(1, target_classes.reshape(-1, 1)).cpu()
    )
    ...
    if (
        ...
        counter_all = -1
        if (
            kwargs["RATIO_config"].no_apgd
            or kwargs["inverse_config"].activate
            or (
                kwargs["RATIO_config"].activate
                and (not kwargs["RATIO_config"].apgd.activate)
            )
            or kwargs["use_generative_model"]
        ):
        ...
        for counter_restarts in range(restarts_outer):
            ...
            print(
                "starting norm denoised",
                denoised_x.reshape(batch_size, -1).norm(p=2, dim=1),
            )
            if not (
                kwargs["use_generative_model"]
                or kwargs["RATIO_config"].activate
                or kwargs["RATIO_config"].no_apgd
            ):
                ...
                assert (
                    kwargs["norm"].lower() in accepted_norms_apgd
                    or not kwargs["RATIO_config"].apgd.activate
                ), f"Norm should be in {accepted_norms_apgd}"
                classifier.eval()
                print("seed is", kwargs["seed"])
                print(
                    "model is DataParallel",
                    isinstance(classifier, torch.nn.DataParallel),
                )
                start = time.time()
                ...
                apgd = APGDAttack(
                    ...
                    else kwargs["inverse_config"].init_apgd_steps,
                    eot_iter=1,
                    rho=kwargs["RATIO_config"].apgd.rho,
                    seed=kwargs["seed"] + counter_restarts,
                    # ToDo: accept not only lower l, or make clearer that only such are accepted
                    device=device,
                    norm=kwargs["norm"].replace("l", "L")
                    if (
                        (
                            "l" in kwargs["norm"]
                            or not kwargs["RATIO_config"].apgd.activate
                        )
                        and not kwargs["inverse_config"].activate
                    )
                    else "L2",
                    eps=kwargs["epsilons"][0]
                    if not kwargs["inverse_config"].activate
                    else kwargs["inverse_config"].init_apgd_eps,
                    verbose=True,
                ...
                adv_curr, all_losses, all_confid, video = apgd.perturb(
                    x_0, target_classes, best_loss=True
                )  # True)
                end = time.time()
                ...
                if not kwargs["inverse_config"].activate:
                    ...
                    objective_vals.append(
                        torch.unsqueeze(classifier_loss_0.detach(), 1).cpu()
                    )
                    logits.append(out_0.gather(1, target_classes.reshape(-1, 1)).cpu())
                    gt_confs = torch.softmax(out_0, dim=1)
                    probs.append(
                        gt_confs.gather(1, target_classes.reshape(-1, 1)).cpu()
                    )

                    probs_all.append(torch.softmax(out_0, dim=1).unsqueeze(dim=2).cpu())
                    l0_, l1_, l2_, l_inf_, LPIPS_, FeatureDist_, MS_SSIM_L1_ = norms(
                        x_0, adv_curr
                    )
                    print("l2", l2_)
                    l_0.append(l0_.cpu())
                    ...
                    print(
                        ...
                        probs[-1],
                        all_confid[
                            torch.arange(all_confid.shape[0]),
                    ...
                    prob_logit_obj_iter_stat = (
                        torch.cat(probs, dim=1),
                        torch.cat(logits, dim=1),
                        all_losses,
                        torch.cat(objective_vals, dim=1),
                        all_confid,
                        None,
                        None,
                    )
                    ...
                    kwargs["inverse_config"].adv_found = torch.zeros_like(
                        kwargs["inverse_config"].penalty_multiplier, dtype=torch.bool
                    )
                    factor_step_armijo = torch.full(
                        (batch_size,), 1.0, device=device, dtype=torch.float
                    )
                    step_found = torch.full_like(
                        kwargs["inverse_config"].penalty_multiplier, L + 1
                    )
                    # RMSProp-related constants
                    kwargs["inverse_config"].eps = float(kwargs["inverse_config"].eps)
                    kwargs["inverse_config"].square_avg = torch.full(
                        x_0.shape,
                    ...
                    print(
                        "probs after init",
                        classifier(denoised_x)
                        .detach()
                        .softmax(1)
                        .gather(1, target_classes.reshape(-1, 1))
                        .cpu(),
                    )

                if (
                    not kwargs["inverse_config"].activate
                    and kwargs["target_classes"] is not None
                ):
                    return (
                        images,
                        denoised_images,
                    ...
                    with torch.enable_grad():
                        ...
                        for _ in range(kwargs["RATIO_config"].loss_diverse_iter):
                            delta = project_perturbation(
                                delta
                                + torch.autograd.grad((w * out_init).sum(), delta)[0],
                                eps=kwargs["epsilons"][0],
                                p=kwargs["norm"],
                                center=x_0,
                            )
            ...
            print(
                "distances init",
                (x_mod_ - x_0).reshape(batch_size, -1).norm(p=2, dim=1),
            )
            ...
            for counter_inner, sigma in enumerate(tqdm(sigmas)):
                labels = torch.empty(batch_size, dtype=torch.long, device=device).fill_(
                    counter_inner
                )
                ...
                if (
                    ...
                    if kwargs["RATIO_config"].adaptive_stepsize:
                        if counter_inner == math.ceil(
                            kwargs["RATIO_config"].apgd.p_next * L
                        ):
                            print(
                                counter_inner,
                                "equal!",
                            ...
                            decrease_count = sum(
                                ...
                                if kwargs["RATIO_config"].apgd.p_current
                                <= i
                                <= kwargs["RATIO_config"].apgd.p_next - 1
                            )

                            condition_1 = decrease_count < kwargs[
                                "RATIO_config"
                            ].apgd.rho * (
                            ...
                            condition_2 = (
                                kwargs["RATIO_config"].apgd.lr_current
                                == kwargs["RATIO_config"].apgd.lr_next
                                and kwargs["RATIO_config"].apgd.f_max_current
                                == kwargs["RATIO_config"].apgd.f_max_next
                            )

                            kwargs["RATIO_config"].apgd.f_max_current = kwargs[
                                "RATIO_config"
                            ].apgd.f_max_next
                            kwargs["RATIO_config"].apgd.p_next += max(
                                kwargs["RATIO_config"].apgd.p_next
                                - kwargs["RATIO_config"].apgd.p_current
                            ...
                            if condition_1 or condition_2:
                                kwargs["RATIO_config"].apgd.lr_current = kwargs[
                                    "RATIO_config"
                                ].apgd.lr_next
                        ...
                        step_size = batch_view(
                            torch.full((batch_size,), step_lr, device=device)
                        )  # * (sigma / sigmas[-1]) ** 2, device=device))
                        ##step_size = batch_view(
                    ...
                    and not kwargs["use_generative_model"]
                ):
                    if first_sample:
                    ...
                    step_size = batch_view(
                        torch.full(
                            (batch_size,),
                            step_lr * (sigma / sigmas[-1]) ** 2,
                            device=device,
                        )
                    )  # 0.01) ** 2 # (sigma / sigmas[-1]) ** 2
                ...
                for k in range(nsigma):
                    counter_all += 1
                    noise = (
                        torch.randn_like(x_mod_) * noise_reduction
                    )  # / torch.log(torch.Tensor([3 + c])).to(device) #noise_reduction

                    if noise_first:
                        x_mod_ += (
                            int(kwargs["use_noise"])
                            * noise
                            * (batch_view(step_size) * 2).sqrt()
                        )
                    ...
                    if kwargs["use_generative_model"]:
                        if target == "gaussian":
                            scores.append(
                                sigma
                                * torch.unsqueeze(
                                    grad.reshape(batch_size, -1).norm(p=2, dim=1), 1
                                )
                            )
                            ...
                            if kwargs["misalignment"]:
                                ...
                                loss = "cosine"
                                def model(x):
                                    return scorenet(x, labels).reshape(batch_size, -1)
                                ...
                                gt_misalignment = att.perturb(
                                    x_mod_, grad.reshape(batch_size, -1), targeted=True
                                ).detach()

                                print("Misalignment is generated!")
                                print(
                                    torch.nn.CosineSimilarity(dim=1, eps=1e-6)(
                                        gt_misalignment.reshape(batch_size, -1),
                        ...
                        with torch.enable_grad():
                            ...
                            if False and kwargs["inverse_config"].activate:
                                step_found.masked_fill_(
                                    (~kwargs["inverse_config"].adv_found)
                                    & kwargs["inverse_config"].is_adv,
                                    counter_inner,
                                )
                                kwargs["inverse_config"].adv_found.logical_or_(
                                    kwargs["inverse_config"].is_adv
                                )
                                exp_decay = kwargs["inverse_config"].lr_reduction ** (
                                    (counter_inner - step_found).clamp_min(0)
                                    / (L - step_found)
                                )
                                ...
                                step_size = batch_view(
                                    step_size_init.clone()
                                ) * batch_view(exp_decay)
                                print(
                                    "exp_decay, c, step_found, L, power, stepsize",
                                    exp_decay,
                                    counter_inner,
                                    step_found,
                                    L,
                                ...
                                print(
                                    "is adv found", kwargs["inverse_config"].adv_found
                                )
                            ...
                            if kwargs["randomized_smoothing"]:
                                ...
                                for i in range(kwargs["noise_iterations"]):
                                    print(i)
                                    x_noisy = denoised_x + kwargs[
                                        "noise_sigma"
                                    ] * torch.randn_like(denoised_x)
                                    out = classifier(x_noisy)
                                    classifier_loss += get_loss(
                                        kwargs_=kwargs,
                                        out_=out,
                                        targets_=sses,
                                        c=counter_inner,
                                ...
                                if kwargs["line_search_config"].type:
                                    _, classifier_loss_none_red = get_loss(
                                        kwargs_=kwargs,
                                        out_=out,
                                        targets_=target_classes,
                                        c=counter_inner,
                            ...
                            if (
                                kwargs["inverse_config"].activate
                                and kwargs["inverse_config"].init_lr_distance
                                is not None
                                and counter_inner == 0
                            ):
                                grad_norm = grad_classifier.reshape(
                                    batch_size, -1
                                ).norm(p=2, dim=1)
                                randn_grad = torch.randn_like(grad_classifier).renorm(
                                    dim=0, p=2, maxnorm=1
                                )
                                grad_classifier = torch.where(
                                    batch_view(grad_norm <= 1e-6),
                                    randn_grad,
                                    grad_classifier,
                                )
                                step_size = batch_view(
                                    init_lr_finder(
                                        x_mod_,
                                        grad_classifier,
                                        distance_function=lambda y: _view_norm(
                                            x=x_mod_, y=y
                                        ),
                            ...
                            if (
                                kwargs["inverse_config"].activate
                                and kwargs["inverse_config"].type == "alma"
                            ):
                                # if c >= L / 2:
                                #    mask_no_grad_norm = (kwargs['inverse_config'].prev_constraint > 1e-3) & (~kwargs['inverse_config'].adv_found)
                                # Using RMSProp
                                kwargs["inverse_config"].square_avg.mul_(
                                    kwargs["inverse_config"].alpha
                                ).addcmul_(
                                    grad_classifier,
                                    grad_classifier,
                                    value=1 - kwargs["inverse_config"].alpha,
                                )
                                avg = (
                                    kwargs["inverse_config"]
                                    .square_avg.sqrt()
                            ...
                            before_norm_grad_classifier = (
                                grad_classifier.detach().clone()
                            )
                            if (
                                kwargs["RATIO_config"].grad_normalization
                                and not kwargs["use_generative_model"]
                            ):
                                lp_norm_name = norms_dict_torch[
                                    kwargs["RATIO_config"].grad_normalization
                                ]
                                lp_norm = batch_view(
                                    grad_classifier.reshape(batch_size, -1).norm(
                                        p=lp_norm_name, dim=1
                                    )
                                )  # .reshape(-1, 1, 1, 1)
                            ...
                            if (
                                False
                                and kwargs["inverse_config"].activate
                                and kwargs["inverse_config"].type
                                == "not_dynamic_penalty"
                            ):
                                # Using Adam
                                momentum = (
                                    kwargs["inverse_config"].beta_1 * momentum
                                    + (1 - kwargs["inverse_config"].beta_1)
                                    * grad_classifier
                                )
                                kwargs["inverse_config"].square_avg.mul_(
                                    kwargs["inverse_config"].beta_2
                                ).addcmul_(
                                    grad_classifier,
                                    grad_classifier,
                                ...
                                momentum_correction = momentum / (
                                    1
                                    - kwargs["inverse_config"].beta_1
                                    ** (counter_inner + 1)
                                )
                                square_avg_correction = kwargs[
                                    "inverse_config"
                                ].square_avg / (
                                    1
                                    - kwargs["inverse_config"].beta_2
                                ...
                                avg = square_avg_correction.sqrt().add_(
                                    torch.tensor(kwargs["inverse_config"].eps)
                                )
                                ...
                                if kwargs["regularizer"] == "l2":
                                    ...
                                    with torch.enable_grad():
                                        ...
                                        grad_regularizer = -torch.autograd.grad(
                                            dist_LPIPS(x_0, x_mod_), x_mod_
                                        )[0].detach()
                                elif kwargs["regularizer"] == "l1":
                                    pass
                                else:
                                    raise NotImplementedError(
                                        "Regularization is only supported for l2 norm!"
                                    )
                            if kwargs["use_generative_model"]:
                                grad_scale_norm = grad.reshape(batch_size, -1).norm(
                                    p=2, dim=1
                                )

                                grad_classifier_norm = grad_classifier.reshape(
                                    batch_size, -1
                                ).norm(p=2, dim=1)

                                print(
                                    "grad classifier, generative model",
                                    list(
                                        zip(
                                            (
                                                batch_view(
                                                    grad_scale_norm
                                                    / grad_classifier_norm
                                                )
                                                * grad_classifier
                                            )
                            ...
                            grad += (
                                batch_view(grad_scale_norm / (5 * grad_classifier_norm))
                                if kwargs["use_generative_model"]
                                else 1
                            ) * grad_classifier + grad_regularizer
                    ...
                    if kwargs["inverse_config"].activate and (
                        kwargs["line_search_config"].type == "wolfe_conditions"
                        or kwargs["line_search_config"].type == "armijo"
                    ):
                        factor_step_armijo = torch.where(
                            kwargs["inverse_config"].is_adv,
                            factor_step_armijo * 0.5,
                            factor_step_armijo,
                        )
                        ...
                        step_size = torch.full(
                            (batch_size,),
                            kwargs["line_search_config"].alpha_current,
                            device=device,
                        )
                        step_size = (step_size * factor_step_armijo).clamp_min(0.1)
                        step_size_start = step_size.clone().detach().cpu()
                        print(
                            "checking wolfe conditions, stepsize is",
                            step_size_start.max(),
                            step_size_start.mean(),
                            step_size_start.median(),
                            step_size,
                        )

                        if not kwargs["line_search_config"].type == "armijo":
                            upper_bound_step_size = torch.full(
                                (batch_size,), np.float("inf"), device=device
                            )
                            lower_bound_step_size = torch.full(
                                (batch_size,),
                        ...
                        total_steps_tensor = (
                            -torch.log2(0.0122 / step_size_start)
                        ).int()
                        ...
                        ##pr = cProfile.Profile()
                        ##pr.enable()
                        mask_1_condition_failed_composed = torch.ones(
                            denoised_x.shape[0], dtype=bool
                        )
                        inverse_mode_threshold_probs_origin = kwargs[
                            "inverse_config"
                        ].inverse_mode_threshold_probs.clone()
                        inverse_mode_penalty_multiplier = kwargs[
                            "inverse_config"
                        ].penalty_multiplier.clone()
                        ...
                        for counter_ in range(
                            total_steps
                        ):  # int(-torch.log2(0.0122/step_size_start.median())):  # ToDo: use variables from the config (100*0.5^13 ~ 0.0122)
                            counter_ += 1
                            print(f"iteration {counter_}/{total_steps}")
                            # ToDo: separate x_mod_temp and denoised_x clearly in code
                            x_mod_temp = (
                                denoised_x[mask_1_condition_failed_composed].clone()
                                + batch_view(step_size)[
                                    mask_1_condition_failed_composed
                                ]
                                * grad[mask_1_condition_failed_composed]
                            )
                            kwargs[
                                "inverse_config"
                            ].inverse_mode_threshold_probs = inverse_mode_threshold_probs_origin[
                                mask_1_condition_failed_composed
                            ]
                            kwargs[
                                "inverse_config"
                            ].penalty_multiplier = inverse_mode_penalty_multiplier[
                                mask_1_condition_failed_composed
                            ]
                            ...
                            _, classifier_loss_temp = get_loss(
                                kwargs_=kwargs,
                                out_=out_temp,
                                targets_=target_classes[
                                    mask_1_condition_failed_composed
                            ...
                            mask_1_condition_failed = -classifier_loss_temp > (
                                ...
                                + step_size[mask_1_condition_failed_composed]
                                * kwargs["line_search_config"].beta_1
                                * (-grad_squared[mask_1_condition_failed_composed])
                            )
                            mask_1_condition_failed_composed = comp_logical_tensors(
                                mask_1_condition_failed_composed,
                                mask_1_condition_failed,
                            )
                            print(
                                "first condition",
                                classifier_loss_temp.shape,
                                mask_1_condition_failed.shape,
                                mask_1_condition_failed_composed.shape,
                                mask_1_condition_failed,
                                mask_1_condition_failed_composed,
                            )  ##, -classifier_loss_temp - (-classifier_loss_none_red[mask_1_condition_failed] + step_size[mask_1_condition_failed] * kwargs['line_search_config'].beta_1 * (-grad_squared)[mask_1_condition_failed]))

                            if not kwargs["line_search_config"].type == "armijo":
                                upper_bound_step_size = torch.where(
                                    mask_1_condition_failed,
                                    step_size,
                                ...
                                step_size = torch.where(
                                    mask_1_condition_failed,
                                    (step_size + lower_bound_step_size) * 0.5,
                                    step_size,
                                )
                            else:
                                step_size[mask_1_condition_failed_composed] = (
                                    step_size[mask_1_condition_failed_composed] * 0.5
                                )  # .unsqueeze(1)[mask_1_condition_failed, 0] = step_size.unsqueeze(1)[mask_1_condition_failed, 0] * 0.5
                                #   step_size = step_size.squeeze()

                            if not kwargs["line_search_config"].type == "armijo":
                                # Second Wolfe condition
                                x_mod_temp = (
                                    denoised_x.clone() + batch_view(step_size) * grad
                                )
                                ...
                                with torch.enable_grad():
                                    ...
                                    classifier_loss_temp_diff = (
                                        classifier_loss_temp.sum()
                                    )
                                    grad_temp = torch.autograd.grad(
                                        classifier_loss_temp_diff, x_mod_temp
                                    )[0]

                                mask_2_condition_failed = (
                                    -grad_temp.reshape(batch_size, -1)
                                    * grad.reshape(batch_size, -1)
                                ).sum(1) < -(
                                    kwargs["line_search_config"].beta_2 * grad_squared
                                )
                                ...
                                lower_bound_step_size = torch.where(
                                    mask_2_condition_failed,
                                    step_size,
                                ...
                                step_size = torch.where(
                                    mask_2_condition_failed
                                    & (upper_bound_step_size != np.float("inf")),
                                    (step_size + upper_bound_step_size) * 0.5,
                                    step_size,
                                )
                                step_size = torch.where(
                                    mask_2_condition_failed
                        ...
                        if not kwargs["line_search_config"].type == "armijo":
                            if (
                                mask_2_condition_failed.any()
                                or mask_1_condition_failed.any()
                            ):
                        ...
                        if not kwargs["line_search_config"].type == "armijo":
                            step_size = torch.where(
                                (~mask_1_condition_failed) & (~mask_2_condition_failed),
                                step_size,
                                step_size_start,
                            )
                        ...
                        kwargs[
                            "inverse_config"
                        ].inverse_mode_threshold_probs = (
                            inverse_mode_threshold_probs_origin.clone()
                        )
                        kwargs[
                            "inverse_config"
                        ].penalty_multiplier = inverse_mode_penalty_multiplier.clone()
                        ##kwargs['inverse_config'].out_0_grad = out_0_grad_.clone()
                        ##kwargs['inverse_config'].out_temp_grad = out_temp_grad_.clone()

                        print("checked wolfe conditions, step_size is", step_size)

                    elif (
                        kwargs["inverse_config"].activate
                        and kwargs["line_search_config"].type == "armijo_momentum_prox"
                    ):
                        # https://www.stat.cmu.edu/~ryantibs/convexopt-S15/lectures/08-prox-grad.pdf
                        ##step_size = step_size.reshape(batch_size, )
                        step_size = torch.full(
                            (batch_size,), 1000.0, device=device
                        )  # kwargs['line_search_config'].alpha_current, device=device)
                        print(
                            "checking armijo_momentum_prox conditions, stepsize is",
                            step_size,
                        )
                        counter_ = 0
                        # k = c + 1
                        ...
                        while counter_ < 20:
                            ...
                            mask_1_condition_failed = -classifier_loss_temp > (
                                ...
                                + (step_size / 2)
                                * (G.reshape(batch_size, -1) ** 2).sum(1)
                            )
                            print(
                                "second term",
                                (
                            ...
                            print(
                                "loss diff",
                                -classifier_loss_none_red + classifier_loss_temp,
                            )
                            print("mask 1, armijo", mask_1_condition_failed)
                            # Here we set beta = 0.5
                            step_size = torch.where(
                                mask_1_condition_failed, step_size * 0.5, step_size
                            )
                            print("t", step_size)
                    ...
                    full_grads.append(
                        before_norm_grad_classifier.detach()
                        .reshape(batch_size, -1)
                        .norm(p=2, dim=1)
                        .unsqueeze(1)
                        if kwargs["RATIO_config"].grad_normalization
                        else grad.detach()
                        .reshape(batch_size, -1)
                        .norm(p=2, dim=1)
                        .unsqueeze(1)
                    ...
                    if kwargs["RATIO_config"].activate:
                        ...
                        if BLS is not None:
                            ...
                            classifier_loss_none_red = get_loss(
                                kwargs_=kwargs,
                                out_=out,
                                targets_=target_classes,
                                reduction="none",
                            ...
                            if first_sample:
                                alpha = (
                                    torch.ones(batch_size).to(device) * 1000
                                )  # 0.1, 1, 1000

                            delta_ = project_perturbation(
                                delta + batch_view(alpha) * (-grad),
                            ...
                            a = (
                                before_norm_grad_classifier.reshape(batch_size, -1)
                                * (delta_).reshape(batch_size, -1)
                            ).sum(1)
                            tau = 0.5
                        ...
                        if BLS == "armijo":
                            ...
                            while mask.any():
                                ...
                                delta_ = project_perturbation(
                                    delta + alpha.reshape(-1, 1, 1, 1) * (-grad),
                                    eps=kwargs["epsilons"][counter_inner],
                                    p=kwargs["norm"],
                                    center=x_0,
                                )
                            ...
                            mask = get_loss(
                                ...
                                targets_=target_classes,
                                reduction="none",
                            ) > classifier_loss_none_red + a + quadratic_term / (
                                2 * alpha
                            )
                            print(
                                "change loss",
                        ...
                        m = (
                            kwargs["RATIO_config"].frank_wolfe.momentum * m
                        ...
                        if (
                            kwargs["RATIO_config"].frank_wolfe.constraint
                            == "intersection"
                        ):
                            ...
                            v = x_0 + maxlin(
                                x_0,
                                -m,
                                kwargs["epsilons"][counter_inner],
                                p=get_norm(kwargs["norm"]),
                            )
                            ...
                            raise NotImplementedError(
                                "FW supports only ball or intersection."
                            )
                        if kwargs["RATIO_config"].frank_wolfe.backtracking_LS.activate:
                            ...
                            # elif BLS == 'Lipschitz'
                                ...
                                if (
                                    kwargs["RATIO_config"].frank_wolfe.backtracking_LS.L
                                    is None
                                ):
                                    kwargs[
                                        "RATIO_config"
                                    ].frank_wolfe.backtracking_LS.L = (
                                        torch.ones(batch_size).to(device) * 1e-6
                                    )
                                    """
                                ...
                                M = (
                                    ...
                                    * kwargs[
                                        "RATIO_config"
                                    ].frank_wolfe.backtracking_LS.eta
                                )
                                a = (
                                    -grad.reshape(batch_size, -1)
                                    * (v - x_mod_).reshape(batch_size, -1)
                                ...
                                print("a", a.shape, a)
                                b = M * ((v - x_mod_).reshape(batch_size, -1) ** 2).sum(
                                    1
                                )
                                print(M.shape)
                                print(
                                    "b",
                                    b.shape,
                                    (v - x_mod_).reshape(batch_size, -1).shape,
                                    ((v - x_mod_).reshape(batch_size, -1) ** 2)
                                    .sum(1)
                                    .shape,
                                    b,
                                )
                                gamma = torch.minimum(
                                    ...
                                    torch.tensor(
                                        kwargs[
                                            "RATIO_config"
                                        ].frank_wolfe.backtracking_LS.gamma_max
                                    ).to(device),
                                )
                                ...
                                mask = get_loss(
                                    kwargs_=kwargs,
                                    out_=classifier(
                                        denoised_x + batch_view(gamma) * (v - x_mod_)
                                    ),
                                    targets_=target_classes,
                                    reduction="none",
                                ) > quadr_approx(
                                ...
                                while mask.any():
                                    ...
                                    M = (
                                        ...
                                        * kwargs[
                                            "RATIO_config"
                                        ].frank_wolfe.backtracking_LS.tau
                                    )
                                    b = (
                                        b * (~mask)
                                        + mask
                                    ...
                                    gamma = gamma * (~mask) + mask * torch.min(
                                        a / b,
                                        torch.tensor(
                                            kwargs[
                                                "RATIO_config"
                        ...
                        if kwargs["inverse_config"].activate and counter_inner == L - 1:
                            ...
                            def threshold_check(x, dist_penalty):
                                return get_loss(kwargs_=kwargs, out_=classifier(x), targets_=target_classes, c=counter_inner, x_0=x_0, x_new=denoised_x, first_sample=first_sample, no_update=True, dist_penalty=dist_penalty, loss_fn_LPIPS=loss_fn)
                            ...
                            dist_penalty = (
                                dist_penalty * (~mask) + mask * dist_penalty * 0
                            )
                            with torch.enable_grad():
                            ...
                            mask_alternative = (threshold < 0) | (
                                threshold - inverse_threshold == 0
                            )
                            step_size = torch.where(
                                batch_view(mask),
                                torch.ones_like(step_size) * 1e-5,
                                step_size,
                            )  # *1000, step_size)
                            ...
                            while mask.any():
                                ...
                                step_size = (
                                    step_size * batch_view(~mask)
                                    + batch_view(mask) * step_size * 1.2
                                ).clamp_min(1e-29)
                    ...
                    if kwargs["regularizer"] == "l1":
                        ...
                        x_mod_ = x_0 + torch.sign(x_mod_ - x_0) * torch.clamp(
                            ...
                            - (
                                0.1 * batch_view(dist_penalty)
                                if kwargs["inverse_config"].norm == "l1"
                                else kwargs["epsilons"][counter_inner]
                            )
                            * step_size,
                            min=0,
                        )  # 10
                    ...
                    if not (
                        kwargs["inverse_config"].activate and counter_inner == L - 1
                    ):
                        denoised_x = (
                            x_mod_.clone()
                            + int(kwargs["use_generative_model"])
                            * int(kwargs["use_noise"])
                            * (sigma**2)
                            * grad
                        )
                    ...
                    if kwargs["project"]:
                        ...
                        print(
                            "Before proj",
                            kwargs["epsilons"][counter_inner - 1],
                            kwargs["norm"],
                            "norm_peturb",
                            _view_norm(
                                x_0 + delta
                        ...
                        if kwargs["norm"] not in ["LPIPS"]:
                            if kwargs["RATIO_config"].activate:
                                ...
                                if kwargs["RATIO_config"].momentum:
                                    delta = project_perturbation(
                                        delta
                                        + kwargs["RATIO_config"].apgd.alpha
                                        * (delta - delta_prev)
                                ...
                                x_mod_ = x_0 + project_perturbation(
                                    x_mod_ - x_0,
                                    eps=kwargs["epsilons"][-1],
                                    p=kwargs["norm"],
                                    center=x_0,
                                ...
                                denoised_x = x_0 + project_perturbation(
                                    denoised_x - x_0,
                                    eps=kwargs["epsilons"][-1],
                                    p=kwargs["norm"],
                                    center=x_0,
                            ...
                            if kwargs["RATIO_config"].activate:
                                ...
                                x_mod_ = project_onto_LPIPS_ball(
                                    loss_fn_LPIPS,
                                    x_mod_,
                                    x_0,
                                    eps=kwargs["epsilons"][-1],
                                )
                                denoised_x = project_onto_LPIPS_ball(
                                    loss_fn_LPIPS,
                                    denoised_x,
                                    x_0,
                                    eps=kwargs["epsilons"][-1],
                        ...
                        and kwargs["RATIO_config"].frank_wolfe.constraint == "ball"
                    ) or kwargs["inverse_config"].activate:
                        ...
                        print(
                            "similarity after",
                            cos(
                                (denoised_x - x_0).reshape(batch_size, -1),
                                kwargs["inverse_config"].out_0_grad,
                            ),
                        )
                    ...
                    if not noise_first:
                        x_mod_ += (
                            int(kwargs["use_noise"]) * noise * np.sqrt(step_size * 2)
                        )
                        if kwargs["RATIO_config"].activate:
                            delta += (
                                int(kwargs["use_noise"])
                    ...
                    if not final_only or (counter_inner + 1 == L and k + 1 == nsigma):
                        images.append(
                            x_0 + delta if kwargs["RATIO_config"].activate else x_mod_
                        )
                    ...
                    iterate_denoised = (
                        x_0 + delta if kwargs["RATIO_config"].activate else denoised_x
                    )
                    ...
                    if kwargs["inverse_config"].activate:
                        ...
                        new_norm = (
                            _view_norm(x_0, iterate_denoised, mean=False)
                            .cpu()
                            .unsqueeze(1)
                        )
                        ...
                        print(
                            "max conf not reached",
                            max_confidence_value_not_reached_threshold,
                        )
                        ##print('least cos', least_cos_value)
                        print("maask norm", (new_norm <= least_norm_value))
                        print(
                            "mask probs",
                            (probs_last_ >= inverse_threshold.to("cpu").unsqueeze(1)),
                        )
                        ...
                        mask = (
                            ...
                            new_norm <= least_norm_value
                        )  ##& (new_cos <= least_cos_value)
                        ...
                        if (counter_inner == (len(sigmas) - 1)) & (k + 1 == nsigma):
                            mask = (
                                torch.full((batch_size,), True, device="cpu")
                                .unsqueeze(0)
                                .reshape(batch_size, 1)
                            )
                    ...
                    if not kwargs["use_generative_model"] or (
                        counter_inner == (len(sigmas) - 1)
                    ) & (k + 1 == nsigma):
                        print("mask is", counter_inner, mask.shape, mask)
                        max_confidence_iterate = (
                            counter_all + 1
                        ) * mask + max_confidence_iterate * (~mask)

                        max_confidence_value = (
                            probs_last_ * mask + max_confidence_value * (~mask)
                        )
                        best_x = iterate_denoised.cpu().clone() * batch_view(
                            mask
                        ) + best_x * batch_view(~mask)
                    ...
                    if kwargs["inverse_config"].activate:
                        max_confidence_value_not_reached_threshold = (
                            probs_last_ * mask_highest_confidence_not_reached_threshold
                            + max_confidence_value_not_reached_threshold
                            * (~mask_highest_confidence_not_reached_threshold)
                        )
                        max_confidence_iterate_not_reached_threshold = (
                            counter_all + 1
                        ) * mask_highest_confidence_not_reached_threshold + max_confidence_iterate_not_reached_threshold * (
                            ~mask_highest_confidence_not_reached_threshold
                        )
                        best_x_not_reached_threshold = (
                            iterate_denoised.cpu().clone()
                            * batch_view(mask_highest_confidence_not_reached_threshold)
                            + best_x_not_reached_threshold
                            * batch_view(~mask_highest_confidence_not_reached_threshold)
                        )

                    def loss_(reduction):
                        return get_loss(kwargs_=kwargs, out_=out, targets_=target_classes, reduction=reduction, x_0=x_0, x_new=denoised_x, first_sample=first_sample, loss_fn_LPIPS=loss_fn).detach()
                    objective_vals.append(torch.unsqueeze(loss_("none"), 1).cpu())
                    ...
                    if (
                        kwargs["RATIO_config"].activate
                        and kwargs["RATIO_config"].adaptive_stepsize
                    ):
                        kwargs["RATIO_config"].apgd.loss_values.append(
                            (delta, loss_("sum"))
                        )
    ...
    if kwargs["inverse_config"].activate:
        best_x = (
            batch_view(least_norm_value < 1e4) * best_x
            + batch_view(least_norm_value >= 1e4) * best_x_not_reached_threshold
        )
    ...
    sample = denoised_images_[-1].reshape(
        denoised_images_[-1].shape[0], *kwargs["imdims"]
    )
    ...
    prob_logit_obj_iter_stat = (
        ...
        torch.cat(logits, dim=1),
        torch.cat(objective_vals, dim=1),
        torch.cat(iterates_norm, dim=1),
        torch.cat(probs_all, dim=2),
        max_confidence_value,
        max_confidence_iterate,
    )
    if kwargs["use_generative_model"]:
...
def anneal_Langevin_dynamics_consistent(
    ...
    **kwargs,
    ...
    for c in range(L):
        ...
        x_mod_ += (
            next_sigma
            * compute_beta(eta, next_sigma / c_sigma)
            * torch.randn_like(x_mod_)
        )
...
def anneal_Langevin_dynamics_inpainting(
    ...
    L = len(sigmas)
    refer_image = (
        refer_image.unsqueeze(1)
        .expand(-1, x_mod_.shape[1], -1, -1, -1)
        .contiguous()
...
def conf_diff_loss(out, y_oh, reduction="mean"):
    ...
    conf_other = torch.max(confidences * (1.0 - y_oh) - y_oh * 1e13, 1)[0]
