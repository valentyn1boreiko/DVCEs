...
class NetworkBlock(nn.Module):
    def __init__(
        self, nb_layers, in_planes, out_planes, block, stride, activation, dropRate=0.0
    ):
        super(NetworkBlock, self).__init__()
        self.layer = self._make_layer(
            block, in_planes, out_planes, nb_layers, stride, activation, dropRate
        )

    def _make_layer(
        self, block, in_planes, out_planes, nb_layers, stride, activation, dropRate
    ):
        layers = []
...
class WideResNet(nn.Module):
    def __init__(
        ...
        self.block1 = NetworkBlock(
            n, nChannels[0], nChannels[1], block, 1, activation, dropRate
        )
        # 2nd block
        self.block2 = NetworkBlock(
            n, nChannels[1], nChannels[2], block, 2, activation, dropRate
        )
        # 3rd block
        self.block3 = NetworkBlock(
            n, nChannels[2], nChannels[3], block, 2, activation, dropRate
        )
...
def WideResNet28x10(
    num_classes=10, activation="relu", dropRate=0.0, return_feature_map=False
):
    return WideResNet(
        28,
        num_classes,
