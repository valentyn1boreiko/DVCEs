...
def maxlin(x_orig, w_orig, eps, p, verbose=True):
    ...
    bs = x_orig.shape[0]
    small_const = 1e-10
    x = x_orig.view(bs, -1).clamp(small_const, 1.0 - small_const)
    w = w_orig.view(bs, -1)
    gamma = x * (w < 0.0) + (1.0 - x) * (w > 0.0)
    delta = gamma.clone()

    w = w.abs()

    ind = gamma == 0.0  # gamma < small_const #gamma == 0.
    # gamma *= (1 - ind.float())
    # delta *= (1 - ind.float())
    ...
    # print(gammacum.max().item(), gammacum.min().item())
    gammacum.clamp_(0.0)  # small_const
    ...
    mucum = torch.cat(
        [torch.zeros([bs, 1], device=x.device), wcum / denominator], dim=1
    )
    if verbose:
    ...
    pmstar = wcum[u, lb - 1] / (eps**p - gammacum[u, lb]).clip(
        small_const
    )  # wcum[u, lb]
    ...
    # print(deltamax)
    delta[ind] = torch.min(
        delta[ind],  # deltamax[ind].unsqueeze(1
...
class APGDAttack:
    ...
    def get_init(self, x):
        if self.norm == "Linf":
            ...
            print(
                f"{get_norm(self.norm)}-norm is",
                (x_adv - x).view(x.shape[0], -1).norm(dim=1, p=get_norm(self.norm)),
            )

        return x_adv - x

    def attack_single_run(
        self, x, y, x_init=None, first_run=False, ODI_annealing_init=False
    ):
        ...
        if x_init is not None:
            ...
            if self.norm == "L1" and self.verbose:
                print(
                    "[custom init] L1 perturbation {:.5f}".format(
            ...
            with torch.enable_grad():
                ...
                for _ in range(self.ODI_steps):
                    if self.use_fw:
                        grad = torch.autograd.grad((w * out_init).sum(), x_adv_temp)[
                            0
                        ].view(x.shape[0], -1)
                    else:
                        grad = torch.autograd.grad((w * out_init).sum(), delta)[0].view(
                            x.shape[0], -1
                        )
                    grad = (grad / grad.norm(p=2, dim=1, keepdim=True)).view_as(x)
                    if self.use_fw:
                        x_adv_temp = self.get_lmo_update(
                            x, x_adv_temp, grad, ODI_stepsize
                        )
                        out_init = self.model(x_adv_temp)
                    else:
                        ...
                        out_init = self.model(x + delta)
                    print("logits distance", (out_init - out_init_0).norm(p=2, dim=1))
            if self.use_fw:
            ...
            with torch.enable_grad():
                ...
                for i in range(20):
                    ...
                    diff = torch.softmax(out_init, dim=1).gather(
                        1, y.view(-1, 1)
                    ) - torch.softmax(out_init_prev, dim=1).gather(1, y.view(-1, 1))
                    best_delta = torch.where(
                        batch_view(
                            torch.softmax(out_init, dim=1).gather(1, y.view(-1, 1))
                            >= best_prob
                        ),
                        delta.detach(),
                        best_delta,
                    )
                    best_prob = torch.where(
                        torch.softmax(out_init, dim=1).gather(1, y.view(-1, 1))
                        >= best_prob,
                        torch.softmax(out_init, dim=1)
                        .gather(1, y.view(-1, 1))
                        .detach(),
                        best_prob,
                    )
                    ...
                    print(
                        "best delta", best_delta.view(x.shape[0], -1).norm(p=2, dim=1)
                    )
                    delta = torch.where(
        ...
        if not self.is_tf_model:
            if self.loss == "ce":
                ...
                def criterion_indiv(probs, y):
                    return -1.0 * loss_nll(probs.log(), y)
            elif self.loss == "ce-targeted-cfts-conf-binary":
                loss_nll = nn.NLLLoss(reduction="none")
                def criterion_indiv(probs, y):
                    return -1.0 * loss_nll(probs.log(), 1 - y)
            elif self.loss == "ce-targeted-cfts":
                if self.dist_regularizer is None:
                    def criterion_indiv(x, y):
                        return -1.0 * F.cross_entropy(x, y, reduction="none")
                else:

                    def criterion_indiv(x, y, x_adv_, x_0_):
                        ce_ = -1.0 * F.cross_entropy(x, y, reduction="none")
                        dist_ = self.dist_regularizer(x_adv_, x_0_).reshape(
                            ce_.shape[0]
                        )
                        return ce_ + dist_
        ...
        if self.return_all_losses_confid:
            if self.second_classifier is not None:
                logits_ = 0.5 * (
                    self.model(x).softmax(1) + self.second_classifier(x).softmax(1)
                )
            else:
        ...
        if self.pgd_mode:
            ...
            step_size = (
                alpha
                * (1 if self.use_fw else self.eps)
        ...
        k = self.n_iter_2 + 0
        if self.norm == "L1" or (
            self.use_fw and type(float_(self.norm)) == float and self.use_largereps
        ):
        ...
        for i in tqdm(range(self.n_iter)):
            ...
            with torch.no_grad():
                ...
                if self.use_fw:
                    ...
                    print(
                        "afw {}-norms update is".format(self.norm),
                        (x_adv_1 - x)
                        .view(x.shape[0], -1)
                    ...
                    if counter3 == k:
                        if self.norm in ["Linf", "L2"] or self.use_fw:
                            ...
                            fl_oscillation = torch.max(
                                fl_oscillation, fl_reduce_no_impr
                            )
    ...
    def perturb(self, x, y=None, best_loss=False, x_init=None):
        ...
        if self.loss != "ce-targeted":
            ...
            acc = y_pred != y
        -1e10 * torch.ones_like(acc).float()
        if self.verbose:
            print(
                "-------------------------- ",
        ...
        if self.use_largereps:
            ...
            iters = [
                0.3 * self.n_iter_orig,
                0.3 * self.n_iter_orig,
                0.4 * self.n_iter_orig,
            ]
    ...
    def dlr_loss_targeted(self, x, y):
        ...
        return -(torch.where(mask, x_sorted[u, -2], x_sorted[u, -1]) - x[u, y]) / (
    ...
    def get_lmo_update(self, x, prev_update, direction, lr):
        ...
        assert (
            condition_assert
            if type(condition_assert) == bool
            else all(condition_assert)
        )
        ...
        if self.fw_constraint == "intersection":
            ...
            raise ValueError(
                "Constraint {} is not implemented".format(self.fw_constraint)
            )
    ...
    def decr_eps_pgd(self, x, y, epss, iters, use_rs=True):
        ...
        if not use_rs:
            ...
            if self.norm == "L1":
                ...
                x_init += L1_projection(x, x_init - x, 1.0 * float(epss[0]))
        ...
        for eps, niter in zip(epss, iters):
            ...
            if x_init is not None:
                if self.norm == "L1":
                    x_init += L1_projection(x, x_init - x, 1.0 * eps)
                elif self.use_fw and type(float_(self.norm)) == float:
            ...
            for counter in range(self.n_restarts):
                ...
                if ind_to_fool.numel() != 0:
                    ...
                    if not self.is_tf_model:
                        if self.second_classifier is not None:
                            output = 0.5 * (
                                self.model(x_to_fool).softmax(1)
                                + self.second_classifier(x_to_fool).softmax(1)
                            )
                        else:
