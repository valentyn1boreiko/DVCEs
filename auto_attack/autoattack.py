...
class AutoAttack():
    def __init__(self, model, norm='Linf', eps=.3, seed=None, verbose=True,
                 ...
                 device='cuda', log_path=None, use_fw=False, second_classifier=None, masks=1):
        self.model = model
        self.second_classifier = second_classifier
        self.norm = norm
        self.use_fw = use_fw
        assert norm in ['Linf', 'L2', 'L1'] or self.use_fw
        if self.use_fw:
            print('using fw mode for AA')
        self.epsilon = eps
        self.seed = seed
        ...
        if not self.is_tf_model:
            ...
            self.apgd = APGDAttack(self.model, n_restarts=5, n_iter=100, verbose=False,
                                   ...
                                   device=self.device, use_fw=self.use_fw)
            ...
            # ToDo: change to default n_restarts=1 and num of classes = 9, and to verbose = False
            self.apgd_targeted = APGDAttack_targeted(self.model, n_restarts=1, n_iter=100, verbose=False,
                                                     ...
                                                     seed=self.seed, device=self.device, use_fw=self.use_fw,
            ...
            from .autopgd_base import APGDAttack_targeted
            self.apgd_targeted = APGDAttack_targeted(self.model, n_restarts=1, n_iter=100, verbose=True,
                                                     eps=self.epsilon, norm=self.norm, eot_iter=1, rho=.75,
                                                     seed=self.seed, device=self.device,
                                                     is_tf_model=True)

        if version in ['standard', 'plus', 'rand', 'binary']:
    ...
    def get_logits(self, x):
        if not self.is_tf_model:
            if self.second_classifier is not None:
                if self.verbose:
                    print('using uniform weights, get_logits')
                # Not logits, but only argmax is used in the code
                return 0.5*(self.model(x).softmax(1) + self.second_classifier(x).softmax(1))
            else:
                return self.model(x)
        else:
        ...
        with torch.no_grad():
            ...
            if self.verbose:
                if self.norm == 'Linf':
                    ...
                    res = (x_adv - x_orig).view(x_orig.shape[0], -1).norm(p=float(self.norm), dim=1)
    ...
    def set_version(self, version='standard'):
        ...
        if version == 'standard':
            ...
            self.apgd_targeted.n_restarts = 9
            self.apgd_targeted.n_target_classes = 1
            self.apgd_targeted.loss = 'ce-targeted-cfts-conf-binary' #'margin-targeted'
